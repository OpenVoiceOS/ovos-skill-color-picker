...
def test_rgb_255_0_0_speaks_red_by_name_en_us():
    ...
    report_color_by_rgb_name_known dialog with color_name "red" -- not just
    match the intent."""
    ...
    try:
        ...
        assert meta["dialog"] == "report_color_by_rgb_name_known", (
            f"expected report_color_by_rgb_name_known, got {meta['dialog']!r} "
...
def test_rgb_it_it_speaks_correct_dialog():
    ...
    try:
        ...
        assert meta["data"]["color_name"].lower() == "rosso", (
            f"expected color_name 'rosso', got {meta['data']!r}"
        )
    finally:
        mc.stop()
